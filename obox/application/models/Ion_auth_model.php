...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @author    Mathew
	 */
	public function register($id_groups,$identity, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		if ($id_groups=="1") {
			$name_groups="admin_group";
		}else{
			$name_groups="default_group";
		}
		// check if the default set in config exists in database
		$query = $this->db->get_where($this->tables['groups'], array('name' => $this->config->item($name_groups, 'ion_auth')), 1)->row();
